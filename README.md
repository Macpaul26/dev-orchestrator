...
**Task 009 is implemented: experience / learning memory.**

Bounded, project-isolated, tamper-evident persistence for episodic experience.
Records survive a restart, are ordered and paged from filenames rather than by
...
Lock ownership lives in the lock's directory NAME, so releasing one can only ever
name your own and an older owner cannot delete a newer owner's lock. Nothing
about a lock's age decides whether it may be taken: a lock is reclaimed when the
...
**The Claude Code bridge is connected.** `dist/bridge/claudeBridge.js` is
the executable the adapter launches: it gives Claude Code exactly four MCP
tools, each forwarded to the orchestrator for authorisation against the
human-approved grant, and disallows every built-in tool. Exercised against
