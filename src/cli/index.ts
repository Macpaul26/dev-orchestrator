...
program
  ...
  .action((opts: { project?: string }) => {
    ...
    line("legacy string-command checks executable: false");
    line("");
    line("CONTROLLED VERIFICATION EXECUTION (Task 005)");
    line("  runs ONLY predefined checks from trusted project configuration");
    line("  executable and arguments are fixed by that configuration, never chosen");
    line("    by an agent, a request, or a model");
    line("  the policy is captured and fingerprinted BEFORE the untrusted run, so a");
    line("    mid-run rewrite blocks execution instead of being executed");
    line("  requires a human-approved grant carrying `verification.execute`");
